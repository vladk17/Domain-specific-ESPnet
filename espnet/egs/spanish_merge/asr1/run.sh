...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ${train_set} ${train_dev} ${recog_set}; do
        remove_longshortdata.sh --maxframes 3000 --maxchars 400 --minchars 20 data/${x}_org data/${x}
    done
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    spm_train --input=data/lang_char/input.txt --vocab_size=${nbpe} --hard_vocab_limit=0 --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000
